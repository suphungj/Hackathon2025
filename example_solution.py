...
def build_wind_speed_model(data):
    ...
    wind_speed_model.fit(
        data[['pressure', 'air_temp', 'ground_temp']], data['windspeed'])
...
def main():
    ...
    ganopolis_information = data[data['city'] == 'GANopolis'].sort_values(
        by=['hour', 'hour_of_day'])
    ...
    for event_number in range(1, 11, 1):
        prediction_data = read_input_data(
            'data/event_'+str(event_number)+'.csv')
        ganopolis_prediction = prediction_data.loc[data['city'] == 'GANopolis'].sort_values(
            by=['hour', 'hour_of_day'])
        # We get the wind speed for the number of hours
        num_hours = 120
        wind_speed = wind_speed_model.predict(
            ganopolis_prediction[['pressure', 'air_temp', 'ground_temp', 'windspeed']], num_hours)
